...
class DBInteraction(object):
    ...
    async def get_constraint_of_table(self, table_name):
    ...
    async def updater_table(self, table, engine, keys, data_iter):
        ...
        upload_dtype = await self.get_table_attr_types(table.name)
        await self.get_constraint_of_table(table.name)
    ...
    async def get_table_attr_types(self, table_name):
    ...
    async def data_upload(self, data_df, table_name, update_table=False):
        ...
        upload_dtype = await self.get_table_attr_types(table_name)
    ...
    async def df_build(self, model_name, question, response):
        df = pd.DataFrame([{
            'model_name': model_name,
            'question': question,
            'answer': response
        }])
        await self.data_upload(df, 'qa_data')